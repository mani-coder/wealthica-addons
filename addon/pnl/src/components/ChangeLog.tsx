...
export default function ChangeLog() {
  ...
  const renderLogItem = (log: (typeof LOGS)[number]) => {
    return (
      <>
  ...
  const items = LOGS.map((log, index) => ({
    ...
    content: renderLogItem(log),
  }));

  return (
    <div ref={containerRef} className="my-4 h-[500px] overflow-y-auto">
      <Timeline titleSpan="10%" items={items} mode="start" />
    </div>
  );
}
