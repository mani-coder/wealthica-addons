...
<div class="w-full h-full overflow-scroll no-scrollbar">
  ...
  <div class="flex flex-col space-y-1 w-full p-2 bg-gray-100 rounded-lg">
    <div class="flex items-center px-2 space-x-3">
      ...
      <div class={`flex-grow flex-col ${pnl.changeRatio >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        <div class="text-base font-medium">
          <Tooltip tooltip="This is the difference between your latest P&L % and P&L % for the selected date.">
        ...
        <div class="text-sm">
          {#if privateMode}
            $--
          {:else}
            <Tooltip tooltip="This is the difference between your latest P&L and the P&L for the selected date.">
              <span>
                {pnl.changeValue >= 0 ? '' : '-'}${Math.abs(Number(pnl.changeValue.toFixed(2))).toLocaleString()}
              </span>
            </Tooltip>
          {/if}
        </div>
    ...
    <div class="flex-col space-y-1 pt-1">
      ...
      <DateValue date={pnl.startDate} value={pnl.startPnl} ratio={pnl.startRatio} {privateMode} />
      <DateValue date={pnl.endDate} value={pnl.endPnl} ratio={pnl.endRatio} {privateMode} />
    </div>
  </div>
