...
export function parseInstitutionsResponse(
  response: any,
  groups?: string[],
  institutions?: string[]
): Account[] {
  ...
  return response
    .filter(
      (institution) =>
        !institutions ||
        !institutions.length ||
        institutions.includes(institution.id)
    )
    .reduce((accounts, instutition) => {
      return accounts.concat(
        instutition.investments
          .filter(
            (account) =>
              (!groups || !groups.length || groups.includes(account.group)) &&
              !account.ignored
          )
          .map((account) => {
            return {
    ...
    .reduce((hash, transaction) => {
      ...
      if (["deposit", "transfer", "withdrawal"].includes(type)) {
        ...
        const account = accounts.find(
          (account) => account.institution === transaction.institution
        );
